...
const assemble = (cls) => {
    const pr = Reflect.getMetadata(reflect_var_1.PROVIDE_TARGET, cls);
    const cl = Reflect.getMetadata(reflect_var_1.CONTROLLER, cls);
    const inj = Reflect.getMetadata(reflect_var_1.INJECT_TARGET, cls);
    const clsObj = new SomeClass();
    console.log(clsObj.useTest);
    const mthods = (0, utils_1.mapRouter)(clsObj)[0];
