...
import { filterFileChangedText, debug, convertPosixToGnu } from './util';
...
// 定义 forkTsc 函数的 options 类型
type ForkTscOptions = {
  cwd?: string;
  onFirstWatchCompileSuccess?: () => void;
  onWatchCompileSuccess?: (fileChangedList: string[]) => void;
  onWatchCompileFail?: () => void;
  onCompileSuccess?: () => void;
};

/**
 *
 * @param {string[]} tscArgs
 * @param {ForkTscOptions} options
 ...
 */
const forkTsc = (tscArgs: string[] = [], options: ForkTscOptions = {}): any => {
  ...
  const totalFileChangedList = new Set<string>();
...
const forkRun = (
  ...
  let onServerReadyCallback:
    | ((
        data: { title: string; debugUrl?: string; data?: any },
        isFirstFork: boolean,
        during: number,
   ...
   * 只在 --keepalive 下有效
   */
  let lastBootstrapStatus: boolean | undefined;
  ...
  function innerFork(isFirstFork = false) {
    ...
    let currentDebugUrl: string | undefined;
    ...
    if (runArgs.includes('--keepalive')) {
      runChild.once('exit', (code: number) => {
  ...
  const killRunningChild = async () => {
    ...
    return new Promise((resolve) => {
      ...
      // 监听进程退出
      runChild.once('exit', () => {
        debug(`child process ${runChild.pid} exited +${Date.now() - now}ms`);
